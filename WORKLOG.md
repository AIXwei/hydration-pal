...
## 2026-07-06 22:18
按用户要求移除成就页的猫咪等级成长卡和勋章墙两个模块（HTML+JS+CSS全清），保留连续达标卡。主进程的等级/勋章计算逻辑保留（toast提示还在用）。

## 2026-07-06 22:54
动画与透视优化（index.html + floating.html + main-window.js 三处同步）：
- 新增 rAF 动画驱动：水位指数平滑补间（每帧走剩余10%），加水时水面缓缓涨起，启动时有从空杯涨到当前水位的开机动画
- 待机漂浮：水面正弦±3、猫相对水面再±4（相位差0.9），周期约3秒，泳圈有荡漾感
- waterFloor 底部透视椭圆随水位在头15个单位内展开，不再从0瞬间弹到480
- 三猫切换从 display 硬切改为 opacity 0.35s 淡入淡出
- 全部动画尊重 prefers-reduced-motion（补间直达+禁用漂浮+禁用过渡）

## 2026-07-06 23:15
修杯底沙面透底：cup.png 沙面中部约5.2万像素在抠白时被杯内空气种子(thresh=42)连带挖穿，页面粉色背景透出像一块错误的水面透视。用原图 杯子.png 按暖色判据(r-b>10)恢复 y650-840 杯内被误挖的沙色像素。合成验证沙面完整、空气区仍透明。
